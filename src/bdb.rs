...
enum PageType {
    HashUnsortedPageType = 2, // Hash pages created pre 4.6. DEPRECATED
    OverflowPageType = 7,
    HashMetadataPageType = 8,
    HashPageType = 13, // Sorted hash page.
    // https://github.com/berkeleydb/libdb/blob/v5.3.28/src/dbinc/db_page.h#L569-L573
    HashOffIndexPageType = 3, // aka HOFFPAGE
}
    ...
    fn read(&mut self) -> Result<Vec<Vec<u8>>, RpmdbError> {
        ...
        for _ in 0..=self.hash_metadata.generic.last_page_no {
            ...
            for idx in hash_page_indexes {
                if page_data.get(idx as usize) != Some(&(PageType::HashOffIndexPageType as u8)) {
                    continue;
                }
...
#[cfg(test)]
mod tests {
    use crate::{bdb::Bdb, DBI};

    #[test]
    fn test_open() {
        let bdb = Bdb::open("testdata/centos7-python35/Packages".parse().unwrap()).unwrap();
