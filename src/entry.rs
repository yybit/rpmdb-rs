...
impl Hdrblob {
    pub(crate) fn from_bytes(data: Vec<u8>) -> Result<Hdrblob> {
        ...
        blob.verify_region(data.clone())?;
        blob.verify_info(data.clone())?;

        Ok(blob)
    }

    pub(crate) fn import(&self, data: Vec<u8>) -> Result<Vec<IndexEntry>> {
        let mut index_entries;
        let dribble_index_entries;
        let mut rdlen;

        let entry: EntryInfo = self.pe_list.get(0).context("empty pe list")?.ei2h();
    ...
    fn verify_region(&mut self, data: Vec<u8>) -> Result<()> {
        let pe = self.pe_list.get(0).context("invalid pe_list index")?;
    ...
    fn verify_info(&self, data: Vec<u8>) -> Result<()> {
        let mut end: i32 = 0;
...
fn region_swab(
    ...
    for (i, pe) in pe_list.iter().enumerate() {
        ...
        let length = if i < pe_list.len() - 1 && TYPE_SIZES.get(info._type as usize) == Some(&-1) {
            ...
            data_length(data.clone(), info._type, info.count, start, data_end)
        };
        if length < 0 {
...
fn data_length(data: Vec<u8>, t: u32, count: u32, start: i32, data_end: i32) -> isize {
    match t {
...
fn strtaglen(data: Vec<u8>, count: u32, start: i32, data_end: i32) -> isize {
...
fn hdrchk_align(t: u32, offset: i32) -> bool {
    let value = offset & (TYPE_ALIGN.get(t as usize).unwrap_or(&0) - 1) as i32;
    value != 0
}
